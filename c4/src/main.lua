...
local function flogic_file_move(from_name, to_name)
  ...
  -- C4Z among the allowed aliases. The documented example uses
  -- leading-slash paths, but a bare filename is also a valid relative
  -- path — and FileMove's own return convention is undocumented. So
  -- try both forms and believe the filesystem, not the call: our flows
  -- never move onto an existing destination, so the destination's
  -- existence proves the move. The updater re-verifies every step by
  -- state regardless.
  ...
  for _, form in ipairs(forms) do
    pcall(function()
      C4:FileMove(flogic_file_store, form[1], flogic_file_store, form[2])
    end)
...
local function flogic_file_read(name, count)
  ...
  local ok, data = pcall(function()
    ...
    -- 2026090811). FileSetPos is available since 1.6.0.
