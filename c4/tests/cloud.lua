...
T.test("cloud: version, link pin, updater asset, no picker (CLOUD-U6)", function()
  T.check_equal(FLOCLOUD_DRIVER_VERSION, "2026090808", "cloud version")
  ...
  local body = JSON.encode({
    {
      tag_name = "c4-v2026090808",
      draft = false,
      prerelease = false,
      assets = {
        {
          name = "flologic_cloud.c4z",
          browser_download_url = "https://github.com/psaab/flologic_HA/releases/download/c4-v2026090808/flologic_cloud.c4z",
        },
      },
    },
    {
      ...
      assets = {
        {
          ...
          browser_download_url = "https://github.com/psaab/flologic_HA/releases/download/c4-v2026090808/flologic_valve.c4z",
        },
      },
    },
  })
  ...
  T.check_equal(release.version, "2026090808", "cloud release version")
...
T.test("cloud: reuse skips remove when the slot rebound live", function()
  ...
  T.check_equal(flocloud_state.valve_slots["200"], 2001, "newcomer mapped despite stale name")
  OnDriverDestroyed()
end)
