...
T.test("updates: install downloads, stages, and triggers on newer release", function()
  local timers = TestHelp.new_fake_timers()
  local releases, store, fakes = install_fixtures("2026090802")
  ...
  op.start()
  T.check(err == nil, "no error, got " .. tostring(err))
  T.check_equal(outcome.attempted, "2026090802", "attempted version")
...
T.test("updates: install follows asset redirects with headers", function()
  local timers = TestHelp.new_fake_timers()
  local releases, _, fakes = install_fixtures("2026090802")
  local hops = {}
  ...
  T.check_equal(outcome.attempted, "2026090802", "attempted after redirect")
