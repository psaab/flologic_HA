...
T.test("valve: plain ON/OFF, BUTTON_ACTION, and RAMP_TO_LEVEL route to open/close", function()
  ...
  -- Tile buttons send press-style actions with no release: press acts.
  ReceivedFromProxy(LIGHT, "BUTTON_ACTION", { BUTTON_ID = 0, ACTION = 1 })
  local press0 = Link.parse(commands_sent(env)[#commands_sent(env)].params)
  T.check_equal(press0.fields.action, "mode_home", "button 0 press opens")
  ReceivedFromProxy(LIGHT, "BUTTON_ACTION", { BUTTON_ID = "1", ACTION = 1 })
  local press1 = Link.parse(commands_sent(env)[#commands_sent(env)].params)
  T.check_equal(press1.fields.action, "mode_shutoff", "button 1 press closes")
  local pairs = #commands_sent(env)
  ReceivedFromProxy(LIGHT, "BUTTON_ACTION", { BUTTON_ID = "1", ACTION = "2" })
  T.check_equal(#commands_sent(env), pairs, "release pair eaten by debounce")
